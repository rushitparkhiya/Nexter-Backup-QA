 ...
 * TC008 — Permissions: non-admin user
 * TC119 — Concurrent click protection (409)
 * TC120 — Re-auth gate on Restore
 * TC121 — Re-auth gate on Wipe
 * TC122 — Re-auth gate on Unpair-site
 * TC208 — SSRF probe: 169.254.169.254 as paired-site URL
 * TC209 — Zip-slip probe: importer with ../../wp-config.php
 */
...
// ── TC008 — Permissions ───────────────────────────────────────────────────────
test('@P0 TC008 — Editor does not see Backup menu in admin sidebar', async () => {
...
test('@P0 TC008 — Editor gets 403 on direct REST hit to /backup/stats', async () => {
  const browser = await chromium.launch();
  const ctx     = await browser.newContext({
    storageState: path.join(__dirname, '..', '.auth', 'editor.json'),
  });
...
test('@P1 TC119 — Second POST /backup/run within 1s returns 409', async ({ page, request }) => {
  const nonce = await getNonce(page);

  // Fire two parallel requests
  const [res1, res2] = await Promise.all([
    apiPost(request, nonce, '/backup/run', { type: 'full' }),
    apiPost(request, nonce, '/backup/run', { type: 'full' }),
  ]);
...
// ── TC120 — Re-auth gate on Restore ──────────────────────────────────────────
test('@P1 TC120 — Restore without confirm_password returns 401', async ({ page, request }) => {
  const nonce  = await getNonce(page);
...
test('@P1 TC120 — Restore with wrong password returns 401 reauth_invalid', async ({ page, request }) => {
  const nonce  = await getNonce(page);
  const backup = await runFullBackup(request, nonce);
...
test('@P1 TC120 — Restore with correct password proceeds (200)', async ({ page, request }) => {
  const nonce  = await getNonce(page);
  const backup = await runFullBackup(request, nonce);
...
// ── TC121 — Re-auth gate on Wipe ─────────────────────────────────────────────
test('@P1 TC121 — POST /backup/wipe without password returns 401', async ({ page, request }) => {
  const nonce = await getNonce(page);
  const res   = await apiPost(request, nonce, '/backup/wipe', {
    settings: 0, destinations: 0,
    // No confirm_password — dry run guard
  });
  expect(res.status()).toBe(401);
  const body = await res.json();
  expect(body.code).toMatch(/reauth_required/);
...
// ── TC122 — Re-auth gate on Unpair-site ──────────────────────────────────────
test('@P1 TC122 — DELETE /backup/paired/{id} without password returns 401', async ({ page, request }) => {
  const nonce = await getNonce(page);
  ...
  if (list.length === 0) {
    // Nothing to unpair — just verify the endpoint rejects without password
    // by passing a fake id
    const res = await apiDelete(request, nonce, '/backup/paired/999', {
      // No confirm_password
    });
    expect([401, 404]).toContain(res.status());
  } else {
    const res = await apiDelete(request, nonce, `/backup/paired/${list[0].id}`, {
      // No confirm_password
    });
    expect(res.status()).toBe(401);
    const body = await res.json();
...
// ── TC208 — SSRF probe ────────────────────────────────────────────────────────
test('@P2 TC208 — Pairing with 169.254.169.254 is rejected', async ({ page, request }) => {
  const nonce = await getNonce(page);
...
test('@P2 TC208 — Pairing with RFC1918 address 10.0.0.1 is rejected', async ({ page, request }) => {
  const nonce = await getNonce(page);
  const res   = await apiPut(request, nonce, '/backup/paired', {
    url:   'http://10.0.0.1',
    code:  'fake-pair-code',
...
test('@P2 TC208 — Pairing with loopback 127.0.0.1 is rejected', async ({ page, request }) => {
  const nonce = await getNonce(page);
  const res   = await apiPut(request, nonce, '/backup/paired', {
    url:   'http://127.0.0.1',
    code:  'fake-pair-code',
...
test('@P2 TC208 — Pairing with gopher:// scheme is rejected', async ({ page, request }) => {
  const nonce = await getNonce(page);
  const res   = await apiPut(request, nonce, '/backup/paired', {
...
// ── TC209 — Zip-slip probe ────────────────────────────────────────────────────
test('@P2 TC209 — Importer zip with ../../wp-config.php entry is skipped', async ({ page, request }) => {
  const nonce = await getNonce(page);

  // Create a malicious zip in memory (fixtures/zip-slip.zip must exist)
  // The fixture must be pre-created — see Appendix B in the code map.
  const fixtureZip = path.join(__dirname, '..', 'fixtures', 'zip-slip.zip');
  if (!fs.existsSync(fixtureZip)) {
    test.skip(true, 'zip-slip.zip fixture not found — see Appendix B');
    return;
  }
